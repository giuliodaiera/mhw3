...
var rest_url_campionato = 'https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=Italy';
...
function onChangeSelectCampionato(){
	...
	switch(valueSelected.toLowerCase()){
		case 'italia':
			rest_url_campionato = 'https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=Italy';
			break;
		case 'inghilterra':
			rest_url_campionato = 'https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=England';
			break;
		case 'germania':
			rest_url_campionato = 'https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=Germany';
			break;
		case 'spagna':
			rest_url_campionato = 'https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=Spain';
			break;
		case 'francia':
			rest_url_campionato = 'https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=France';
			break;
		default:
			rest_url_campionato = 'https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=Italy';
	}
...
function selectCampionato(event){
	...
	fetch(rest_url_campionato).then(onResponse, onError).then(onCampionatoSelected);
}
...
function searchGiocatore(event){
	...
	if(!player_value){
		console.log('Nessun testo inserito');
		return;
	}

	console.log('Eseguo ricerca: ' + player_value);

	let rest_url_giocatore = 'https://apiv2.apifootball.com/?action=get_players&player_name=' + player_value + '&APIkey=' + APIkey;
	console.log('URL: ' + rest_url_giocatore);

	fetch(rest_url_giocatore).then(onResponse, onError).then(onGiocatoreResponse);
}
